        ...
        if ($template->status && $template->isItem) {
            if (!in_array("-1", $editState)) {
                ...
                if ($template->documentControlType->documentControlTypeName != 'date')
                    ...
                    if ($documentType->documentTypeId == 36) {
                        $lastSalaryDate = Calendar::model()->findLastSalaryDate();
//                        throw new Exception($lastSalaryDate->date);
                        $datePickerConfig['options'] = array(
                            'dateFormat' => "yy-mm-dd",
                            'minDate' => "$lastSalaryDate->date"
                        );
                    }
