...
if ($model->documentDocumentTemplateField) {
	...
	foreach ($model->documentDocumentTemplateField as $item) {
		...
		if (!isset($currentWorkflowState->workflowCurrent)) { // ตรวจสอบว่าเป็น แบบร่างรึมั้ย
			if ($documentWorkflowModel->isFinished == 1) { // ตรวจสอบว่าเอกสาร จบการทำงานรึยัง
				$canEdit = false;
			} else {
				$canEdit = true;
			}
		...
		if (isset($template->documentControlType)) {
			if ($template->documentControlType->documentControlTypeName == "file") {
				...
				if (!empty($item->value)) {
					if (strpos($item->value, ".pdf")) { //ถ้าเป็น pdf ให้ fancy box โหลด class pdf แทน
