...
<?php
echo $form->dropDownList($model, 'elearningId', CHtml::listData(Elearning::model()->findAll(array(
			'order'=>'title')), 'elearningId', 'title'), array(
	'class'=>'input-block-level',
	'prompt'=>'--- Select Elearning ---'));
?>
...
<?php if(!$choiceModels): ?>
	<?php for($i = 1; $i <= 4; $i++): ?>
		...
		<p class="alert alert-info">
			<label class="radio">
				<?php
				echo CHtml::radioButton('isCorrect', false, array(
					'value'=>$i));
				?>
			...
			<?php
			echo $form->textField($choiceModel, '[' . $i . ']title', array(
				'class'=>'input-block-level'));
			?>
	...
	<?php foreach($choiceModels as $key=> $choiceModel): ?>
		...
		<p class="alert alert-info">
			<label class="radio">
				...
				<?php
				echo CHtml::radioButton('isCorrect', ($choiceModel->isCorrect) ? true : false, array(
					'value'=>$choiceId));
				?>
				<?php echo $choiceModel->getAttributeLabel('isCorrect'); ?>
			</label>
			...
			<?php echo $form->labelEx($choiceModel, 'title'); ?>
			<?php
			echo $form->textField($choiceModel, '[' . $choiceId . ']title', array(
				'class'=>'input-block-level'));
			?>
			<?php echo $form->error($choiceModel, 'title'); ?>
