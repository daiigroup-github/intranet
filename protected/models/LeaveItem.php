...
class LeaveItem extends CActiveRecord
{
    ...
    //custom
    public function getLeaveItemTimeTypeArray($type = null, $empId = null)
    {
        if (!isset($empId)) {
            $emp = Employee::model()->findByPk(Yii::app()->user->id);
        } else {
            $emp = Employee::model()->findByPk($empId);
        }
        ...
        if ($type != 3) {
            if ($emp->branchId == 1) {
                $result = array(
                    ...
                    self::LEAVE_TIME_RANGE_15_18 => '3 ชั่วโมง เวลา 15.00-16.00',
                    self::LEAVE_TIME_RANGE_13_17 => '4 ชั่วโมง เวลา 13.00-17.00',
                    self::LEAVE_TIME_RANGE_14_18 => '4 ชั่วโมง เวลา 14.00-18.00',
                );
            } else {
                $result = array(
                    self::LEAVE_TIME_RANGE_FULLDAY => 'เต็มวัน',
    ...
    public function leaveItemTimeTypeText($leaveTimeRange, $empId = null)
    {
        $ltr = $this->getLeaveItemTimeTypeArray(null, $empId);
