...
class EmailSend //extends Controller
{
	...
	public function mailReserve($toEmail, $toName, $movieName, $showDate, $creatorName, $reservedUrl, $isManager = FALSE, $isCancel = FALSE, $cancelRemark = NULL, $isCallReserve = false)
	{
		$message = new YiiMailMessage();
		$message->view = 'theater_reserve';
		$message->setBody(array(
			"toName"=>$toName,
			"movieName"=>$movieName,
			"showDate"=>$showDate,
			...
			'isCallReserve'=>$isCallReserve
			), 'text/html', 'utf-8');

		//$message->message->setBody($body, 'text/html');
//		$message->message->setBody($body, 'text/plain', 'utf-8');

		$message->subject = "จดหมายแจ้งเตือนรายการจองดูหนัง";
		$message->addTo($toEmail);
		//$message->from   = ($this->adminEmail);
		$message->setFrom(array(
			'No-Reply@daiigroup.com'=>'แจ้งเตือน จองดูหนัง Intranet'));
		if(Yii::app()->getParams()->sendEmail)
		{
			Yii::app()->mail->send($message);
		}
